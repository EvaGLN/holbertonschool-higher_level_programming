...
- [Real Python Serialization](/rltoken/dCh_kqSXRNy5zjTznUki6w)
- [Real Python: Working With JSON Data in Python](/rltoken/OD6iwbpQ_i9HPoAGXJEGLQ)
- [Python's pickle documentation](/rltoken/IibAgnq4_yiGI9w0uqIxxw)
- [Corey Schafer on Pickle](/rltoken/8Kk_654pW2ZQPuiRNgWGCA)
- [CSV to JSON in Python](/rltoken/ZSsuXXF6b30nGMDUveJYkw)
- [Python XML ElementTree Guide](/rltoken/fvfT2Y6UVND9sfIgzqIe4w)
- [Socket Programming Guide](/rltoken/F-DQ6YzeR9vV0QWWhlOpFg)
...
* `@classmethod`
`deserialize(cls, filename)`: This class method will take a filename as its parameter. Using the `pickle` module, it will load and return an instance of the `CustomObject` from the provided filename.
...
1. Define a function named `convert_csv_to_json` that takes the CSV filename as its parameter and writes the JSON data to `data.json`.
...
1. The function should return `True` if the conversion was successful.
2. Handle exceptions, such as `file not found`. Function should return `False` in this case.
3. Save your work in `task_02_csv.py`.
